...
async function searchDict(pattern) {
    const dict = fs.createReadStream('words_alpha.txt');
    const dictLineStream = readline.createInterface({
        input: dict,
        crlfDelay: Infinity
    });

    const paternArr = pattern.split("");
    for await (const line of dictLineStream) {
        const match = search(line.split(""), paternArr);
        if (match) {
            console.log(match);
            break;
        }
